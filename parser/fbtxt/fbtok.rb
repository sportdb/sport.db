...
def read( path )
  ...
  nodes.each do |node|
    ...
    if type == :h1
      ...
      ## use tokenize line by line
      tree = []
      lines = txt.split( "\n" )
      lines.each_with_index do |line,i|
          ##  todo - strip inline comments?
          ##           check if empty and comment line processing needed here?
          ##             already handled by outline reader!
          next if line.strip.empty? || line.strip.start_with?( '#' )
          tokens, errors = parser.tokenize_with_errors( line, debug: debug? )
